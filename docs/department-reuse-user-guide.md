...
| 顺序 | 表 | 主要来源 | 填写重点 |
| --- | --- | --- | --- |
| 01 | 年度排课窗口表 | 人工维护或后台新增 | 用 `2026暑假`、`2026秋季` 这类“年度+窗口”管理实际排课窗口 |
| 02 | 课节表 | 后台按年度窗口批量生成 | 维护日期、星期、上午/下午/晚上、是否可用和不可排原因 |
...
如果要导入 ERP，先从 ERP 下载待排课班级的空白课表或导入模板，再把程序排课结果中的课次 ID、日期、时间、班级编码、教师 ID、教室 ID、课程 ID 等字段写回导入模板。ERP 产品对应表要提前维护好，否则课程编码和版本无法稳定回写。
